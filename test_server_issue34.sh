...
make build # fails because root but creates the manifest.mk file
echo "Ignore the error above... docker inside docker and permissions stuff... doesnt matter"
chgrp -R docker $GOPATH/src/github.com/bblfsh/python-driver       && \
chmod g+rwx -R $GOPATH/src/github.com/bblfsh/python-driver        && \
su prebblfsh -c "make build" && \
